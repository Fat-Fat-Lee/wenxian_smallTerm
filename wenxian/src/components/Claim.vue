      ...
      <el-row :gutter="20">
        ...
        <el-col :span="16">
          <div style="margin:10px">
            <el-input  placeholder="请按照文献名检索" v-model="queryInfo.query" clearable style="width:90%" ></el-input>
            ...
            <el-card class="zcx_card">
              <el-col>
                ...
                <el-row class="info_row0" type="flex">
                  <el-tag  size="medium" style="margin: 0 3px" v-if="r.rcondition===1">审核中</el-tag>
                  <el-tag  type="danger" size="medium" style="margin: 0 3px" v-if="r.rcondition===3">已驳回</el-tag>
                  <el-tag  type="info" size="medium" style="margin: 0 3px" v-if="r.rcondition===0">未认领</el-tag>
                  <el-tag  type="success" size="medium" style="margin: 0 3px" v-if="r.rcondition===2">已认领</el-tag>
                </el-row>
                <el-row class="info_row0" type="flex" >
                ...
                <el-row class="info_row1" style="color:#696969" type="flex">
                  ...
                  <div v-for="(a,index3) in r.author" :key="index3" style="margin: 5px 5px">
                    {{a}}.
                  </div>
                </el-row>
                <el-row class="info_row2">
                  <el-button type="primary" size="mini" plain @click="download_(r)" style="margin:10px 3px">下载文件</el-button>
                  <el-button type="success" size="mini" plain @click="make_claim(r)" style="margin:10px 3px" v-if="r.rcondition===0">认领文献</el-button>
                  <el-button type="success" size="mini" plain @click="make_claim(r)" style="margin:10px 3px" v-if="r.rcondition===3">重新申请</el-button>
                </el-row>
              </el-col>
<!--              <el-col :span="4">-->
...
export default {
  ...
  data(){
    return{
      ...
      researchlist:[
        // {aID:"1",name:"Electroencephalogram-Based Preference Prediction Using Deep Transfer Learning",author:["zcx","ljw"],public_time:"2021-10-25",pic: require("../assets/CSS/defaultpage.png"),tag:"CS",subjects:["s1","s2"],rcondition:"审核中",download_url:""},
        // {aID:"2",name:"Product Recommendation in Case-based Reasoning",author:["ljw","zcx"],public_time:"2021-10-24test",pic: require("../assets/CSS/defaultpage.png"),tag:"SE",subjects:["s3","s4"],rcondition:"未认领",download_url:""},
        // {aID:"3",name:"Mobile Application Based on Dijkstra’s Algorithm, to Improve the Inclusion of People with Motor Disabilities Within Urban Areas",author:["ljw","zcx"],public_time:"2021-11-15",pic: require("../assets/CSS/defaultpage.png"),tag:"SE",subjects:["s5","s6"],rcondition:"已驳回",download_url:""},
      ],
  ...
  methods:{
    ...
    make_claim(r){
      this.$confirm('是否确认是您的研究成果？', '提示', {
        confirmButtonText: '确认',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        //传递信息给后端
        this.claim(r,this.uID);
        this.fetch(this.uID);
        this.$message({
          type: 'success',
          message: '认领文献成功!'
    ...
    async claim(r) {
      ...
      await new Promise((resolve) => {
        this.$http
          .post("/claimLiterature/", JSON.stringify({ uID, aID, name }))
          .then((res) => {
            console.log(res);
            if (res.data.success === false) {
              this.$message.error("认领研究失败");
    ...
    async fetch(uID) {
      await new Promise((resolve) => {
        this.$http
          .post("/getClaimList/", JSON.stringify({uID}))
          .then((res) => {
            console.log(res);
            if (res.data.success !== true) {
              this.$message.error(res.data.message);
            ...
            console.log(res.data);
            console.log(res.data.data.Claimlist);
            this.researchlist=res.data.data.Claimlist;
            this.$message({
              message: "个人文献列表获取成功",
              showClose: true,
              type: "success",
